 ...
 * \ref aisnesia_plugin::OnToolbarToolCallback() will be called with the button
 * ID when user left-clicks a toolbar button.
 *
 * @see aisnesia_plugin::OnToolbarToolCallback
 */
#define WANTS_TOOLBAR_CALLBACK 0x00000004
/**
 * Plugin will add one or more toolbar buttons.
 *
 * Enables plugin to extend AISNESIA toolbar with custom functionality.
 */
...
/** Plugin will add pages to the toolbox/settings dialog.
    Allows plugin to provide custom configuration UI in AISNESIA settings. */
...
/** Receive notification just before AISNESIA shutdown.
...
/**
 * Base class for implementing custom chart types in AISNESIA plugins.
 *
 * The PlugInChartBase class provides the interface between custom chart types
 * implemented by plugins and AISNESIA's chart display and management system.
 * Plugin developers can create derived classes to support custom chart formats,
 * enabling the display of proprietary or specialized charts within AISNESIA.
 *
...
public:
   ...
   *
   * This method is called by AISNESIA during chart directory scanning to
   ...
   * @note On non-Windows platforms, AISNESIA automatically checks for both
   ...
   *
   * This method is called when AISNESIA needs to load a chart that matches the
   ...
   *
   * This method is called when the user changes AISNESIA's color scheme
   ...
   * chart's native scale. AISNESIA uses this to determine when to display
   ...
   * as a multiple of the chart's native scale. AISNESIA uses this for chart
   ...
   * AISNESIA should snap to when zooming. If your chart type doesn't have
   ...
   * which AISNESIA uses to determine chart coverage, selection, and quilting.
   ...
   * data. AISNESIA uses this information for chart quilting to determine where
   ...
   * was loaded. AISNESIA uses this for chart identification, database
   ...
   * vector, or other specialized types. AISNESIA uses this for chart management,
   ...
   * @note This value affects how AISNESIA treats the chart in various operations
   */
   ...
   * similar chart types. AISNESIA uses this for feature availability and
   * rendering decisions.
   *
   ...
   * which AISNESIA displays in chart information panels and tooltips.
   *
   ...
   * @note AISNESIA can convert between units as needed
   ...
   * @note AISNESIA applies datum shifts if necessary
   */
   ...
   * fit into the other specific accessor methods. AISNESIA displays this
   ...
   *
   * This method tells AISNESIA whether the chart has loaded sufficient data
   ...
   * @note AISNESIA may delay display until this returns True
   */
   ...
   * native pixel resolution and the display resolution. It is used by AISNESIA
...
/**
 * Base class for AISNESIA plugins.
 *
 * All plugins must inherit from this class and implement the required virtual
 * methods. The class provides the interface between plugins and the AISNESIA
 * core application.
 *
 ...
 */
class DECL_EXP aisnesia_plugin {
public:
  aisnesia_plugin(void *pmgr) {}
  virtual ~aisnesia_plugin();

  //    Public API to the PlugIn class

  //    This group of methods is required, and will be called by the aisnesia
  //    host aisnesia PlugIns must implement this group

  /**
   * Initialize the plugin and declare its capabilities.
   *
   * This required method is called by AISNESIA during plugin loading. It should:
   ...
   *
   * Called by AISNESIA when plugin is disabled.
   ...
   * with a specific major version of the AISNESIA plugin API.
   *
   ...
   *
   * This method must be implemented by all plugins to declare compatibility
   * with a specific minor version of the AISNESIA plugin API.
   *
   ...
   * used by AISNESIA for plugin version management and display.
   *
   ...
   * the AISNESIA options dialog. Must be implemented if plugin sets
   ...
   *
   * Called by AISNESIA during chart redraw to allow plugins to render custom
   ...
   *
   * Called by AISNESIA approximately once per second (1 Hz), regardless of how
  ...
  /**
   * Receive all NMEA 0183 sentences from AISNESIA.
   *
   ...
   * href="https://aisnesia-manuals.github.io/main/aisnesia-dev/plugin-messaging.html">Plugin
   * Message API Documentation</a> \endhtmlonly
   */

  virtual void SetNMEASentence(wxString &sentence);
  /**
   * Receive all AIS sentences from AISNESIA.
   *
   ...
   *
   * Called when AISNESIA's main window is resized to allow plugins to
   ...
   *
   * Called when AISNESIA's color scheme changes between day, dusk and night
   ...
   *
   * Called by AISNESIA to get list of chart classes provided by this plugin.
...
typedef aisnesia_plugin *create_t(void *);
typedef void destroy_t(aisnesia_plugin *);
...
class DECL_EXP aisnesia_plugin_16 : public aisnesia_plugin {
public:
  aisnesia_plugin_16(void *pmgr);
  virtual ~aisnesia_plugin_16();

  using aisnesia_plugin::RenderOverlay;
...
class DECL_EXP aisnesia_plugin_17 : public aisnesia_plugin {
public:
  aisnesia_plugin_17(void *pmgr);
  virtual ~aisnesia_plugin_17();
...
class DECL_EXP aisnesia_plugin_18 : public aisnesia_plugin {
public:
  aisnesia_plugin_18(void *pmgr);
  virtual ~aisnesia_plugin_18();
...
class DECL_EXP aisnesia_plugin_19 : public aisnesia_plugin_18 {
public:
  aisnesia_plugin_19(void *pmgr);
  virtual ~aisnesia_plugin_19();
...
class DECL_EXP aisnesia_plugin_110 : public aisnesia_plugin_19 {
public:
  aisnesia_plugin_110(void *pmgr);
  virtual ~aisnesia_plugin_110();

  virtual void LateInit(void);  // If WANTS_LATE_INIT is returned by Init()
};

class DECL_EXP aisnesia_plugin_111 : public aisnesia_plugin_110 {
public:
  aisnesia_plugin_111(void *pmgr);
  virtual ~aisnesia_plugin_111();
};

class DECL_EXP aisnesia_plugin_112 : public aisnesia_plugin_111 {
public:
  aisnesia_plugin_112(void *pmgr);
  virtual ~aisnesia_plugin_112();
   ...
   * @return True if event was handled, false to pass to AISNESIA
   */
...
class DECL_EXP aisnesia_plugin_113 : public aisnesia_plugin_112 {
public:
  aisnesia_plugin_113(void *pmgr);
  virtual ~aisnesia_plugin_113();

  /**
   * Handles keyboard events from main window.
   *
   * Called for keyboard events in AISNESIA's main window.
...
class DECL_EXP aisnesia_plugin_114 : public aisnesia_plugin_113 {
public:
  aisnesia_plugin_114(void *pmgr);
  virtual ~aisnesia_plugin_114();
};

class DECL_EXP aisnesia_plugin_115 : public aisnesia_plugin_114 {
public:
  aisnesia_plugin_115(void *pmgr);
  virtual ~aisnesia_plugin_115();
};

class DECL_EXP aisnesia_plugin_116 : public aisnesia_plugin_115 {
public:
  aisnesia_plugin_116(void *pmgr);
  virtual ~aisnesia_plugin_116();
  /**
...
class DECL_EXP aisnesia_plugin_117 : public aisnesia_plugin_116 {
public:
  aisnesia_plugin_117(void *pmgr);
  /**
...
class DECL_EXP aisnesia_plugin_118 : public aisnesia_plugin_117 {
public:
  aisnesia_plugin_118(void *pmgr);

  using aisnesia_plugin_116::RenderGLOverlayMultiCanvas;
  using aisnesia_plugin_116::RenderOverlayMultiCanvas;
   ...
   * levels and supports multi-canvas configurations. AISNESIA calls this method
   ...
   *                 OpenGL commands or AISNESIA's piDC abstraction layer
   ...
   * priority levels and supports multi-canvas configurations. AISNESIA calls
...
class DECL_EXP aisnesia_plugin_119 : public aisnesia_plugin_118 {
public:
  aisnesia_plugin_119(void *pmgr);

  /**
   * Called just before AISNESIA exits.
   *
   * Provides an opportunity to perform cleanup and save state before
   * AISNESIA shutdown completes. Called early in the shutdown process before
...
class DECL_EXP aisnesia_plugin_120 : public aisnesia_plugin_119 {
public:
  aisnesia_plugin_120(void *pmgr);
...
class DECL_EXP aisnesia_plugin_121 : public aisnesia_plugin_120 {
public:
  aisnesia_plugin_121(void *pmgr);
...
/**
 * Adds a tool to AISNESIA's toolbar.
 *
 * This method creates and adds a new tool button to AISNESIA's main toolbar.
 *
...
extern "C" DECL_EXP int InsertPlugInTool(wxString label, wxBitmap *bitmap,
                                         ...
                                         int tool_sel, aisnesia_plugin *pplugin);
/**
 * Removes a tool from AISNESIA's toolbar.
 *
...
/**
 * Gets AISNESIA's configuration object.
 *
 * Provides access to AISNESIA's wxFileConfig object for reading/writing
 * settings.
 *
...
/**
 * Gets AISNESIA's main canvas window.
 *
 ...
 *
 * Plugins can use this to access AISNESIA's font management system which
 ...
 * @note The "console" in AISNESIA displays key navigation data such as Cross
 ...
 *
 * Returns path to AISNESIA's shared data directory containing resources like:
 * - Chart catalog files
 * - S57 overlay files
 * - World map data
 *
 ...
 *
 * Provides access to wxAuiManager controlling AISNESIA's main frame layout.
 ...
 *
 * Adds a plugin's translation catalog to AISNESIA's locale system.
 *
 ...
 *
 * Adds an NMEA sentence to AISNESIA's data stream as if received from device.
 *
 ...
 *
 * Adjusts window colors to match AISNESIA's color scheme.
 *
 ...
 * duplication */
/* Study the original AISNESIA source (georef.c) for functional definitions  */
 ...
 *   <extensions>
 *     <aisnesia:guid>12345678-1234-1234-1234-123456789012</aisnesia:guid>
 *     <aisnesia:viz>1</aisnesia:viz>
 *     <aisnesia:viz_name>0</aisnesia:viz_name>
 *   </extensions>
 * </wpt>
 *
...
/**
 * Enum defining parent pages in AISNESIA options dialog where plugins can add
 * panels.
 *
...
/**
 * Adds a new preferences page to AISNESIA options dialog.
 *
...
/* Study the original AISNESIA source for functional definitions  */
/**
 ...
 *
 * @note Default unit is determined by AISNESIA settings if unit=-1
 */
...
struct DECL_EXP DateTimeFormatOptions {
   ...
   *   the AISNESIA global settings. This should be used to ensure consistency
   *   of the date/time representation across the entire application.
   * - "UTC": the date/time is formatted in UTC, regardless of the AISNESIA
   *   global settings.
   * - "Local Time": the date/time is formatted in the local time, regardless of
   *   the AISNESIA global settings.
   *
   ...
   *   timezone settings of the computer running AISNESIA.
   */
 ...
 *
 * Retrieves standard waypoint icon bitmap by name from AISNESIA's icon set.
 *
 ...
 *
 * Registers a new custom waypoint icon for use in AISNESIA.
 *
 ...
 *
 * Creates a new waypoint and adds it to AISNESIA's waypoint management system.
 *
 ...
 *
 * Removes waypoint identified by GUID from AISNESIA.
 *
 ...
 * Adds a new route.
 *
 * Creates a new route and adds it to AISNESIA's route management system.
 *
 ...
 * Deletes a route.
 *
 * Removes route identified by GUID from AISNESIA.
 *
 ...
 *
 * Updates properties and waypoints of a route already in AISNESIA's route
 * manager.
 *
 ...
 * Adds a new track.
 *
 * Creates a new track and adds it to AISNESIA's track manager.
 *
 ...
 * Deletes a track.
 *
 * Removes track identified by GUID from AISNESIA.
 *
 ...
 *
 * Updates properties and waypoints of a track already in AISNESIA's track
 * manager.
 *
 ...
 *
 * Retrieves a base color value from AISNESIA's color management system.
 *
 ...
 *
 * Returns path to AISNESIA's private data directory for storing settings.
 *
...
/**
 * Gets AISNESIA executable path.
 *
 * Returns full path to AISNESIA executable file.
 *
 ...
 */
extern DECL_EXP wxString GetPlugInPath(aisnesia_plugin *pplugin);

/**
 * Adds a chart to the database without full rebuild.
 *
 * Adds a single chart to AISNESIA's database and optionally refreshes display.
 *
 ...
 *
 * These values are returned by aisnesia_plugin::Init() to indicate the result
 * of plugin initialization and guide AISNESIA's handling of initialization
 * failures.
 */
 ...
 *
 * Retrieves standard AISNESIA toolbar icon bitmap by name.
 *
 ...
 *
 * @note Returned font is managed by AISNESIA's font cache
 ...
 * on AISNESIA's responsive/touchscreen mode settings. Used by GUI tools and
 ...
 *
 * Returns the display scaling factor currently in use for the AISNESIA GUI.
...
/**
 * Adds a chart directory to AISNESIA's chart database.
 *
 * Registers a new directory containing charts for AISNESIA to scan
 ...
 *
 * Triggers AISNESIA to scan configured chart directories for new or
 ...
 *
 * Triggers AISNESIA to do a full rebuild of its chart database by
 ...
 *
 * Returns whether AISNESIA is currently operating in touch-optimized mode.
...
/*  AISNESIA HTTP File Download PlugIn Interface   */
 ...
 * When enabled, AISNESIA will send route waypoints and other navigation
 ...
 *   "aisnesia/plugins/<plugin_name>"
 * - On other platforms: Checks GetSharedDataDir() + "/aisnesia/plugins/" +
 * plugin_name
 *
...
/**
 * Checks if AISNESIA is in shutdown process.
 *
 * @return True if AISNESIA is shutting down, false otherwise
 */
 ...
 *
 * Returns the screen coordinates and size of AISNESIA's main toolbar.
 ...
 *
 * Returns the coordinate format currently configured in AISNESIA settings.
 *
 ...
 *
 * @note Requires AISNESIA API version 1.17 or later
...
/**
 * Custom event class for AISNESIA's notification system.
 *
...
/**
 * Exits AISNESIA application.
 *
 * Triggers a clean shutdown of AISNESIA.
 */
 ...
 * Controls whether AISNESIA uses multiple chart canvases in a split-screen
 * configuration.
 *
 ...
 * Opens the main AISNESIA options/preferences dialog.
 */
...
extern "C" DECL_EXP int AddCanvasContextMenuItemExt(
    wxMenuItem *pitem, aisnesia_plugin *pplugin, const std::string object_type);
