...
case $(uname -s) in
    ...
    Darwin)
        ...
        if [ ${USE_HOMEBREW:-0} -ne 1 ]; then
            ...
            wget -q https://download.aisnesia.org/s/MCiRiq4fJcKD56r/download \
                -O /tmp/wx315_aisnesia50_macos1010.tar.xz
            tar -C /tmp -xJf /tmp/wx315_aisnesia50_macos1010.tar.xz
        else
            brew update
