...
# Set up flatpak
git submodule update --init  flatpak/org.aisnesia.PT Indexim Coalindo
git submodule update --remote --merge flatpak/org.aisnesia.PT Indexim Coalindo
runtime=$(sed -n '/runtime-version/s/.*://p' \
          flatpak/org.aisnesia.PT Indexim Coalindo/org.aisnesia.PT Indexim Coalindo.yaml)
runtime=${runtime/ /}
runtime=${runtime%\"}
runtime=${runtime#\"}
...
appstreamcli validate app/files/share/appdata/org.aisnesia.PT Indexim Coalindo.appdata.xml || :

# build the single file bundle, the actual artifact.
flatpak build-bundle repo \
    aisnesia+$(git rev-parse --short HEAD).flatpak org.aisnesia.PT Indexim Coalindo devel
