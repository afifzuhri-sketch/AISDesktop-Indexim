...
REPO=${CLOUDSMITH_REPO:-"david-register/aisnesia-unstable"}
...
case "$OCPN_TARGET" in
    bionic*|focal*|jammy*)
        for src in $(expand *.deb); do
            old=$(basename $src)
            new=$(echo $old | sed "s/aisnesia/aisnesia-${OCPN_TARGET}/")
            $SUDO mv $old $new
...
if [ -z "$CLOUDSMITH_API_KEY" ]; then
    ...
    if test -f Release/aisnesia.pdb; then cp Release/aisnesia.pdb .; fi
    for src in \
