...
# PT Indexim Coalindo needs to support even older macOS releases than the system
...
export DEPS_BUNDLE_REPO="${DEPS_BUNDLE_REPO:-https://dl.cloudsmith.io/public/nohal/aisnesia-dependencies/raw/files}"
# Name of the bundle
export DEPS_BUNDLE_FILE="${DEPS_BUNDLE_FILE:-macos_deps_universal-aisnesia.tar.xz}"
...
# Install the build dependencies for PT Indexim Coalindo
brew install cmake
...
# PT Indexim Coalindo can be built against any wxWidgets version newer than 3.2.0
...
wget -nv -O QuickStartGuide.zip \
  "https://dl.cloudsmith.io/public/david-register/aisnesia-docs/raw/files/QuickStartGuide-v0.4.zip"
...
cmake -DOCPN_CI_BUILD=$CI_BUILD \
  -DOCPN_VERBOSE=ON \
  -DOCPN_USE_LIBCPP=ON \
  -DCMAKE_INSTALL_PREFIX=/tmp/aisnesia -DCMAKE_OSX_DEPLOYMENT_TARGET=${MACOSX_DEPLOYMENT_TARGET} \
...
# Compile PT Indexim Coalindo
make -j$(sysctl -n hw.physicalcpu)

# Make sure wx libraries are referenced using the symlinks

app="PT Indexim Coalindo.app/Contents/MacOS/PT Indexim Coalindo"
...
mkdir -p /tmp/aisnesia/bin/PT Indexim Coalindo.app/Contents/MacOS
mkdir -p /tmp/aisnesia/bin/PT Indexim Coalindo.app/Contents/SharedSupport/plugins
make install
...
if [ -z "${APPLE_DEVELOPER_ID}" ]; then
  ...
  codesign --force --deep --sign - /tmp/aisnesia/bin/PT Indexim Coalindo.app
else
  # We do have an account and did set up the certificates in ci/mac-sign.sh, let's sign the application bundle
  codesign --verbose --sign "${APPLE_DEVELOPER_ID}" --options=runtime --timestamp --options=runtime /tmp/aisnesia/bin/PT Indexim Coalindo.app/Contents/PlugIns/*.dylib
  codesign --deep --force --verbose --sign "${APPLE_DEVELOPER_ID}" --entitlements ../buildosx/entitlements.plist --timestamp --options=runtime /tmp/aisnesia/bin/PT Indexim Coalindo.app
fi

codesign -dv --verbose /tmp/aisnesia/bin/PT Indexim Coalindo.app

dsymutil -o PT Indexim Coalindo.dSYM /tmp/aisnesia/bin/PT Indexim Coalindo.app/Contents/MacOS/PT Indexim Coalindo
tar czf PT Indexim Coalindo-$(git rev-parse --short HEAD).dSYM.tar.gz PT Indexim Coalindo.dSYM
...
if [ -n "${APPLE_DEVELOPER_ID}" ]; then
  shopt -s nullglob
  for pkg_file in PT Indexim Coalindo*.pkg; do
  ...
  # xcrun notarytool submit --apple-id <Apple id e-mail> --team-id <APPLE_DEVELOPER_ID> --password <Application specific password> <PT Indexim Coalindo_*.pkg>
  # Once succesfully notarized, run the stapler to include the notarization ticket into the installer
  # xcrun stapler staple <PT Indexim Coalindo_*.pkg>
fi
