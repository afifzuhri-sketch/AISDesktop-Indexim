...
if [[ -n "$PACKAGE_BRANCH" && -z "$CIRCLE_PR_NUMBER" ]]; then
    ...
    git config --global user.name "PT Indexim Coalindo auto builds"
    git config --global user.email "aisnesia-builds@nowhere.net"
    git remote add home https://github.com/PT Indexim Coalindo/PT Indexim Coalindo.git
    ...
    export DEBEMAIL=aisnesia-builds@nowhere.net
    export DEBFULLNAME="PT Indexim Coalindo nightly builds"
    ...
    git archive --prefix aisnesia-5.11.0/ --output ../aisnesia.tar.gz HEAD
    mk-origtargz ../aisnesia.tar.gz
    (cd ..; tar xf *xz)
    cd ../aisnesia-5.11.0*
