    ...
    if (pic->m_enabled && pic->m_init_state) {
      if (pic->m_cap_flag & WANTS_PLUGIN_MESSAGING) {
        switch (pic->m_api_version) {
          case 106: {
            auto* ppi = dynamic_cast<aisnesia_plugin_16*>(pic->m_pplugin);
            if (ppi) ppi->SetPluginMessage(id, body);
            break;
          }
          case 107: {
            auto* ppi = dynamic_cast<aisnesia_plugin_17*>(pic->m_pplugin);
            if (ppi) ppi->SetPluginMessage(id, body);
            break;
          }
          case 108:
          ...
          case 120:
          case 121: {
            auto* ppi = dynamic_cast<aisnesia_plugin_18*>(pic->m_pplugin);
            if (ppi) ppi->SetPluginMessage(id, body);
            break;
          }
          default:
    ...
    if (pic->m_enabled && pic->m_init_state) {
      if (pic->m_cap_flag & WANTS_NMEA_EVENTS) {
        switch (pic->m_api_version) {
          ...
          case 120:
          case 121: {
            auto* ppi = dynamic_cast<aisnesia_plugin_117*>(pic->m_pplugin);
    ...
    if (pic->m_enabled && pic->m_init_state) {
      if (pic->m_cap_flag & WANTS_MOUSE_EVENTS) {
        switch (pic->m_api_version) {
          ...
          case 120:
          case 121: {
            auto* ppi = dynamic_cast<aisnesia_plugin_112*>(pic->m_pplugin);
      ...
      if (pic->m_cap_flag & WANTS_KEYBOARD_EVENTS) {
        {
          switch (pic->m_api_version) {
            ...
            case 120:
            case 121: {
              auto* ppi = dynamic_cast<aisnesia_plugin_113*>(pic->m_pplugin);
    ...
    if (pic->m_enabled && pic->m_init_state) {
      if (pic->m_cap_flag & WANTS_PRESHUTDOWN_HOOK) {
        switch (pic->m_api_version) {
          ...
          case 120:
          case 121: {
            auto* ppi = dynamic_cast<aisnesia_plugin_119*>(pic->m_pplugin);
