...
void PluginPaths::InitFlatpakPaths() {
  ...
  const string flathome = m_home + "/.var/app/org.aisnesia.AISNESIA";
  m_userLibdir = flathome + "/lib";
  m_user_bindir = flathome + "/bin";
  m_user_datadir = flathome + "/data";

  m_libdirs.push_back(flathome + "/lib");
  m_libdirs.push_back("/app/extensions/lib/aisnesia");
  m_libdirs.push_back("/app/lib/aisnesia");
  ...
  m_datadirs.push_back("/app/extensions/share/aisnesia/plugins");
  m_datadirs.push_back("/app/share/aisnesia/plugins");
}
...
void PluginPaths::InitLinuxPaths() {
  ...
  if (len > 0) {
    ...
    base_plugin_paths.push_back(expand(path + "/../lib/aisnesia"));
    if (g_BasePlatform->GetOSDetail()->osd_arch.find("64") != string::npos) {
      base_plugin_paths.push_back(expand(path + "/../lib64/aisnesia"));
    } else {
      base_plugin_paths.push_back(expand(path + "/../lib32/aisnesia"));
    }
  }
#endif

  const char* const envdirs = getenv("AISNESIA_PLUGIN_DIRS");
  string dirlist = envdirs ? envdirs : "~/.local/lib/aisnesia";
  ...
  for (auto& dir : m_bindirs) {
    ...
    size_t pos = dir.rfind("/lib/aisnesia");
    if (pos == string::npos) {
      pos = dir.rfind("/lib64/aisnesia");
    }
...
void PluginPaths::InitApplePaths() {
  ...
  const string mac_home = m_home + "/Library/Application Support/AISNESIA";
  ...
  // m_libdirs.push_back("/Applications/AISNESIA.app/Contents/Plugins");
  m_bindirs = m_libdirs;

  m_datadirs.push_back(m_user_datadir);
  m_datadirs.push_back("/Applications/AISNESIA.app/Contents/PlugIns");
}
...
void PluginPaths::InitAndroidPaths() {
  ...
  m_userLibdir =
      platform_dir + "/manPlug";  //("/data/user/0/org.aisnesia.aisnesia");
  m_user_bindir =
      platform_dir + "/manPlug";  //("/data/user/0/org.aisnesia.aisnesia");
  m_user_datadir =
