...
CompatOs::CompatOs() : _name(PKG_TARGET), _version(PKG_TARGET_VERSION) {
  ...
  if (getenv("AISNESIA_COMPAT_TARGET") != 0) {
    _name = getenv("AISNESIA_COMPAT_TARGET");
    ...
    // CompatOS and CompatOsVersion in aisnesia.conf/.ini file.
    _name = g_compatOS;
...
static bool win_entry_set_install_path(struct archive_entry* entry,
  ...
  if (ocpn::endswith(path, ".dll") || ocpn::endswith(path, ".exe")) {
    ...
    wxFileName fn(installPaths["share"].c_str(),
                  "");   // should point to .../aisnesia/plugins
    fn.RemoveLastDir();  // should point to ".../aisnesia
...
static bool linux_entry_set_install_path(struct archive_entry* entry,
  ...
  if (g_bportable) {
    ...
    if (ocpn::startswith(location, "share") &&
        ocpn::startswith(suffix, "aisnesia/plugins/")) {
      slashpos = suffix.find_first_of("aisnesia/plugins/");
    ...
    if (ocpn::startswith(location, "lib") &&
        ocpn::startswith(suffix, "aisnesia/")) {
    ...
    if (ocpn::startswith(location, "lib") &&
        ocpn::startswith(suffix, "aisnesia/") && ocpn::endswith(suffix, ".so")) {
      is_library = true;
    }
  }
...
static bool apple_entry_set_install_path(struct archive_entry* entry,
  ...
  const string base = PluginPaths::GetInstance()->Homedir() +
                      "/Library/Application Support/AISNESIA";
...
static bool android_entry_set_install_path(struct archive_entry* entry,
  ...
  if ((location == "lib") && ocpn::startswith(suffix, "aisnesia")) {
    auto parts = split(suffix, "/");
    if (parts.size() == 2) suffix = parts[1];
    is_library = true;
  }

  if ((location == "share") && ocpn::startswith(suffix, "aisnesia")) {
    auto parts = split(suffix, "aisnesia/");
    if (parts.size() == 2) suffix = parts[1];
  }

  /// storage/emulated/0/android/data/org.aisnesia.aisnesia/files/aisnesia/plugins/oesenc_pi/data/LUPPatch3.xml
  string dest = installPaths[location] + "/" + suffix;
 ...
 * oesenc_pi_ubuntu_10_64/usr/local/share/aisnesia/plugins/oesenc_pi/README.
...
/** Remove all empty dirs found from root containing string "aisnesia". */
static void PurgeEmptyDirs(const std::string& root) {
  if (!wxFileName::IsDirWritable(root)) return;
  if (ocpn::tolower(root).find("aisnesia") == std::string::npos) return;
  wxDir rootdir(root);
