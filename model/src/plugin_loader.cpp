 ...
 * when aisnesia is started it means that the attempt to load the file
...
std::string PluginLoader::GetPluginVersion(
  ...
  auto p = dynamic_cast<aisnesia_plugin_117*>(pic->m_pplugin);
  if (p) {
...
static void ProcessLateInit(PlugInContainer* pic) {
  if (pic->m_cap_flag & WANTS_LATE_INIT) {
    ...
    auto ppi = dynamic_cast<aisnesia_plugin_110*>(pic->m_pplugin);
    if (ppi) ppi->LateInit();
  }
}
  ...
  if (pic->m_enabled && pic->m_init_state) {
    if (pic->m_cap_flag & INSTALLS_TOOLBOX_PAGE) {
      switch (pic->m_api_version) {
        ...
        case 121: {
          if (pic->m_pplugin) {
            auto ppi = dynamic_cast<aisnesia_plugin_19*>(pic->m_pplugin);
            if (ppi) {
              ppi->OnSetupOptions();
              auto loader = PluginLoader::GetInstance();
...
bool PluginLoader::LoadPluginCandidate(const wxString& file_name,
  ...
  // aisnesia. For this reason we must check that we didn't already load this
  // plugin
  bool loaded = false;
  ...
  // "system" plugin directory, e.g. /usr/lib/aisnesia on linux
...
bool PluginLoader::UpdatePlugIns() {
  ...
  for (const auto& pic : plugin_array) {
    ...
    // image...
    if (pic->m_pplugin) {
      auto ppl = dynamic_cast<aisnesia_plugin*>(pic->m_pplugin);
      if (!ppl) {
        pic->m_pplugin = nullptr;
        pic->m_init_state = false;
      }
...
bool PluginLoader::CheckPluginCompatibility(const wxString& plugin_file) {
  ...
  // hard code the file name into the AISNESIA sources. This makes it easier to
...
PlugInContainer* PluginLoader::LoadPlugIn(const wxString& plugin_file,
  ...
  aisnesia_plugin* plug_in = create_plugin(this);
  ...
  switch (api_ver) {
    case 105:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin*>(plug_in);
      break;

    case 106:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_16*>(plug_in);
      break;

    case 107:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_17*>(plug_in);
      break;

    case 108:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_18*>(plug_in);
      break;

    case 109:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_19*>(plug_in);
      break;

    case 110:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_110*>(plug_in);
      break;

    case 111:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_111*>(plug_in);
      break;

    case 112:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_112*>(plug_in);
      break;

    case 113:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_113*>(plug_in);
      break;

    case 114:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_114*>(plug_in);
      break;

    case 115:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_115*>(plug_in);
      break;

    case 116:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_116*>(plug_in);
      break;

    case 117:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_117*>(plug_in);
      break;

    case 118:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_118*>(plug_in);
      break;

    case 119:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_119*>(plug_in);
      break;

    case 120:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_120*>(plug_in);
      break;

    case 121:
      pic->m_pplugin = dynamic_cast<aisnesia_plugin_121*>(plug_in);
      break;
  ...
  if (auto p = dynamic_cast<aisnesia_plugin_117*>(plug_in)) {
