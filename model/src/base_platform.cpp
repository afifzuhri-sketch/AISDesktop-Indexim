 ...
 * \file
 *
 * Implement base_platform.h -- AISNESIA Platform specific support utilities
 */
...
#include <initguid.h>
#include <psapi.h>
#include "setupapi.h"  // presently stored in aisnesia/src
#endif
...
static bool checkIfFlatpacked() {
  ...
  return id == "org.aisnesia.AISNESIA";
}
...
static wxString GetLinuxDataPath() {
  ...
  while (tokens.HasMoreTokens()) {
    ...
    if (!dir.EndsWith("/aisnesia/plugins")) {
      dir += "/aisnesia/plugins";
    }
...
wxString& AbstractPlatform::DefaultPrivateDataDir() {
  if (m_PrivateDataDir.IsEmpty()) {
    ...
    if (getenv("XDG_CONFIG_HOME")) {
      ...
      config_home += "/.var/app/org.aisnesia.AISNESIA/config";
    }
    m_PrivateDataDir = config_home + "/aisnesia";

#elif defined __WXOSX__
    m_PrivateDataDir =
        std_path.GetUserConfigDir();  // should be ~/Library/Preferences
    appendOSDirSlash(&m_PrivateDataDir);
    m_PrivateDataDir.Append("aisnesia");
#else
    if (getenv("OCPN_TEST_HOMEDIR"))
      m_PrivateDataDir = getenv("OCPN_TEST_HOMEDIR");
    else
      m_PrivateDataDir = std_path.GetUserDataDir();  // should be ~/.aisnesia
#endif
...
wxString AbstractPlatform::GetWinPluginBaseDir() {
  ...
  path.Normalize();
  winPluginDir = path.GetFullPath() + "\\aisnesia\\plugins";
...
wxString& AbstractPlatform::GetPluginDir() {
  if (m_PluginsDir.IsEmpty()) {
    ...
    m_PluginsDir = std_path.GetPluginsDir();  // linux:   {prefix}/lib/aisnesia
...
#ifdef __ANDROID__
    // something like: data/data/org.aisnesia.aisnesia
...
wxString& AbstractPlatform::GetConfigFileName() {
  if (m_config_file_name.IsEmpty()) {
...
#ifdef __WXMSW__
    m_config_file_name = "aisnesia.ini";
    ...
    m_config_file_name.Append("aisnesia");
    appendOSDirSlash(&m_config_file_name);
    m_config_file_name.Append("aisnesia.ini");
#elif defined FLATPAK
    m_config_file_name = GetPrivateDataDir();
    m_config_file_name.Append("/aisnesia.conf");
    // Usually ~/.var/app/org.aisnesia.AISNESIA/config/aisnesia.conf
#else
    m_config_file_name = std_path.GetUserDataDir();  // should be ~/.aisnesia
    appendOSDirSlash(&m_config_file_name);
    m_config_file_name.Append("aisnesia.conf");
#endif
    ...
    if (g_bportable) {
      ...
      m_config_file_name += "aisnesia.ini";
#else
      m_config_file_name += "aisnesia.conf";
#endif
    }
...
bool BasePlatform::InitializeLogFile() {
  ...
  wxFileName LibPref(mlog_file);  // starts like "~/Library/Preferences/aisnesia"
  LibPref.RemoveLastDir();        // takes off "aisnesia"
  LibPref.RemoveLastDir();        // takes off "Preferences"

  mlog_file = LibPref.GetFullPath();
  appendOSDirSlash(&mlog_file);

  mlog_file.Append("Logs/");  // so, on OS X, aisnesia.log ends up in
                              // ~/Library/Logs which makes it accessible to
                              // Applications/Utilities/Console....
#endif

  // create the aisnesia "home" directory if we need to
  ...
  if (true != wxHomeFiledir.DirExists(wxHomeFiledir.GetPath()))
    if (!wxHomeFiledir.Mkdir(wxHomeFiledir.GetPath())) {
      wxASSERT_MSG(false, "Cannot create aisnesia home directory");
      return false;
    }

  // create the aisnesia "log" directory if we need to
  ...
  if (true != wxLogFiledir.DirExists(wxLogFiledir.GetPath())) {
    if (!wxLogFiledir.Mkdir(wxLogFiledir.GetPath())) {
      wxASSERT_MSG(false, wxString("Cannot create aisnesia log directory: ") +
                              wxLogFiledir.GetPath());
      return false;
    }
  }

  mlog_file.Append("aisnesia.log");
  ...
  if (::wxFileExists(mlog_file)) {
    if (wxFileName::GetSize(mlog_file) > 1000000) {
      ...
      //  established.
      large_log_message = ("Old log will be moved to aisnesia.log.log");
...
wxString AbstractPlatform::GetPluginDataPath() {
  ...
  if (isFlatpacked()) {
    dirs = "~/.var/app/org.aisnesia.AISNESIA/data/aisnesia/plugins";
  } else if (osSystemId & wxOS_UNIX_LINUX) {
    dirs = GetLinuxDataPath();
  } else if (osSystemId & wxOS_WINDOWS) {
    dirs = GetWinPluginBaseDir();
  } else if (osSystemId & wxOS_MAC) {
    dirs = "/Applications/AISNESIA.app/Contents/SharedSupport/plugins;";
    dirs +=
        "~/Library/Application Support/AISNESIA/Contents/SharedSupport/plugins";
  }
#endif
