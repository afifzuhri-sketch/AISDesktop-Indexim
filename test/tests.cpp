...
static void ConfigSetup() {
  const auto config_orig = fs::path(TESTDATA) / "aisnesia.conf";
  const auto config_path = fs::path(CMAKE_BINARY_DIR) / "aisnesia.conf";
...
public:
  ...
  static std::string GetSocketPath() {
    ...
    if (getenv("OCPN_TEST_HOMEDIR"))
      path = wxFileName(getenv("OCPN_TEST_HOMEDIR"), "aisnesia-ipc");
    else
      path = wxFileName("~/.aisnesia", "aisnesia-ipc");
