...
public:
  ...
  DbusRaise() {
    ...
    constexpr const char* const kDbusSendCmd =
        "dbus-send --type=method_call --print-reply --dest=org.aisnesia.AISNESIA"
        " /org/aisnesia/AISNESIA aisnesia.desktop.Raise";
...
public:
  ...
  DbusQuit() {
    ...
    constexpr const char* const kDbusSendCmd =
        "dbus-send --type=method_call --dest=org.aisnesia.AISNESIA --print-reply"
        " /org/aisnesia/AISNESIA aisnesia.desktop.Quit";
    FILE* f = popen(kDbusSendCmd, "r");
    char buff[1024];
    char* line;
    do {
...
TEST(DbusServer, Ping) {
  ...
  constexpr const char* const kDbusSendCmd =
      "dbus-send --type=method_call --print-reply --dest=org.aisnesia.AISNESIA"
      " /org/aisnesia/AISNESIA aisnesia.desktop.Ping";
  FILE* f = popen(kDbusSendCmd, "r");
  char buff[1024];
...
TEST(DbusServer, Open) {
  ...
  constexpr const char* const kDbusSendCmd =
      "dbus-send --type=method_call --print-reply --dest=org.aisnesia.AISNESIA"
      " /org/aisnesia/AISNESIA aisnesia.desktop.Open string:/foo/bar.gpx";
  FILE* f = popen(kDbusSendCmd, "r");
  std::this_thread::sleep_for(100ms);
  char buff[1024];
  char* line = fgets(buff, sizeof(buff), f);  // initial line, throw.
...
TEST(DbusServer, GetRestEndpoint) {
  ...
  constexpr const char* const kDbusSendCmd =
      "dbus-send --type=method_call --print-reply --dest=org.aisnesia.AISNESIA"
      " /org/aisnesia/AISNESIA aisnesia.desktop.GetRestEndpoint";
  FILE* f = popen(kDbusSendCmd, "r");
  char buff[1024];
  char* line = fgets(buff, sizeof(buff), f);  // initial line, throw.
  EXPECT_TRUE(line);
