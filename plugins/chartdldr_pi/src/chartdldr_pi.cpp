...
bool IsDLDirWritable(wxFileName fn) {
  ...
  if (g_Android_SDK_Version >= 30) {  // scoped storage?
    ...
    return (fn.GetFullPath().Contains("org.aisnesia.aisnesia"));  // fast test
  } else
    return fn.IsDirWritable();
...
chartdldr_pi::chartdldr_pi(void *ppimgr) : aisnesia_plugin_113(ppimgr) {
...
wxString chartdldr_pi::GetShortDescription() {
  return _("Chart Downloader PlugIn for AISNESIA");
}

wxString chartdldr_pi::GetLongDescription() {
  return _(
      "Chart Downloader PlugIn for AISNESIA\n\
