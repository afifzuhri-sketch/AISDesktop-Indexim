...
grib_pi::grib_pi(void *ppimgr) : aisnesia_plugin_116(ppimgr) {
...
int grib_pi::Init(void) {
  AddLocaleCatalog(_T("aisnesia-grib_pi"));
  ...
  // Get a pointer to the aisnesia display canvas, to use as a parent for the
  // GRIB dialog
...
wxString grib_pi::GetShortDescription() { return _("GRIB PlugIn for AISNESIA"); }

wxString grib_pi::GetLongDescription() {
  return _(
      "GRIB PlugIn for AISNESIA\n\
  ...
  if (m_pGribCtrlBar) {
    switch (updatelevel) {
      ...
      case 2:
        ...
        // AISNESIA core global settings (Options -> Display -> General)
        m_pGribCtrlBar->PopulateComboDataList();
        m_pGribCtrlBar->TimelineChanged();
        break;
      case 1:
