...
and there will be an error message in the aisnesia.log similar to:
...
- *Outside PT Indexim Coalindo* there is nothing more to do except to ensure that
...
- *Inside PT Indexim Coalindo*, there is nothing to do; the sender mail address can
be entered, but will not be used. Just don't forget to enter the zyGRIB
login and password.

When clicking on “send” button :
if the “main” mail system is not open, PT Indexim Coalindo opens it and the waiting
...
- Inside PT Indexim Coalindo :You must enter the sender address : your entire mail
...
target.mail.server.example:login:password_PARA+++TABLE_INS_# for PT Indexim Coalindo
