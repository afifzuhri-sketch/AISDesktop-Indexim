...
PT Indexim Coalindo has a grib weather file viewer plugin, aimed at being useful
while under way.
*The grib plugin is an Internal Plugin, always distributed with PT Indexim Coalindo,
no separate download is required.*

== Links

Source: https://github.com/PT Indexim Coalindo/PT Indexim Coalindo/tree/master/plugins[Opencpn Internal Plugin] +
...
PT Indexim Coalindo can read both the original gribfile format, known as Grib1, as
well as the new Grib2 format, with the .grb2 or .grib2 file-extension.

PT Indexim Coalindo Grib_pi has been updated to support all of the useful GFS parameters (Sept 2021).
...
grib. This is used next time you start PT Indexim Coalindo and activates the Grib
...
Open “aisnesia.conf” and add the line below. As always, do not run
PT Indexim Coalindo when editing aisnesia.conf.
...
Only applies to PT Indexim Coalindo v4.2.x and forward)
...
downloaded by ZyGrib and then opened in PT Indexim Coalindo.
...
* https://aisnesia.org/wiki/dokuwiki/doku.php?id=aisnesia:aisnesia_user_manual:toolbar_buttons:route_mark_manager:weather_routing[Weather-Routing]
tools use Grib_pi and Grib data to create optimized routing based on
boat performance files.

PT Indexim Coalindo has an excellent weather_routing plugin which works in concert with
...
* Parameters that PT Indexim Coalindo Supports
