...
//  Read("ResponsiveGraphics", &g_bresponsive);
  ...
  if (GetNumberOfEntries()) {
    ...
    while (bCont) {
      ...
      // https://github.com/AISNESIA/AISNESIA/issues/3042, this effectively gets
      // rid of them.
    ...
    while (bCont) {
      ...
      if (!dirname.IsEmpty()) {
        /*     Special case for first time run after Windows install with sample
         chart data... We desire that the sample configuration file aisnesia.ini
         ...
         Data directory For instance, if the (sample) aisnesia.ini file should
         ...
         ChartDir1=c:\Program Files\aisnesia\SampleCharts\\MaptechRegion7
        ...
        if (wpt_dups > 0) {
          OCPNMessageBox(
              ...
              wxString::Format("%d " + _("duplicate waypoints detected "
                               ...
                               wpt_dups),
              _("AISNESIA Info"), wxICON_INFORMATION | wxOK, 10);
        }
#endif
      }
      delete pSet;
