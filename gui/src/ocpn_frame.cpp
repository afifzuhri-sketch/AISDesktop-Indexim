...
//    MyFrame::OnPreferencesResultTimer ), NULL, this );
  ...
  ais_callbacks.confirm_stop_track = []() {
    int r = OCPNMessageBox(
        ...
        _("AISNESIA Info"), wxYES_NO | wxCENTER, 60);
    return r == wxID_YES;
  };
...
#if defined(__WXGTK__) || defined(__WXQT__)
  wxIcon app_icon(aisnesia);  // This comes from aisnesia.xpm inclusion above
  SetIcon(app_icon);
#endif
...
void MyFrame::StartRebuildChartDatabase() {
  ...
  if (ChartDirArray.GetCount()) {
    ...
    if (g_NeedDBUpdate == 1) {
      wxString msg1(
          _("AISNESIA needs to update the chart database from config file "
            "entries...."));

      OCPNMessageDialog mdlg(gFrame, msg1, wxString(_("AISNESIA Info")),
    ...
    wxGenericProgressDialog *pprog = new wxGenericProgressDialog(
        _("AISNESIA Chart Update"), line, 100, NULL, wxPD_SMOOTH);
...
void MyFrame::OnCloseWindow(wxCloseEvent &event) {
  ...
  if (b_inCloseWindow) {
    //            wxLogMessage(_T("aisnesia::MyFrame re-entering
    //            OnCloseWindow"));
    return;
  }
  ...
  */
  ...
  wxLogMessage("aisnesia::MyFrame exiting cleanly.");
...
void MyFrame::RegisterGlobalMenuItems() {
...
#ifndef __WXOSX__
  nav_menu->AppendSeparator();
  nav_menu->Append(ID_MENU_OQUIT, _menuText(_("Exit AISNESIA"), "Ctrl-Q"));
#endif
  ...
  help_menu->Append(wxID_ABOUT, _("About AISNESIA"));
  help_menu->Append(wxID_HELP, _("AISNESIA Help"));
...
void MyFrame::ProcessOptionsDialog(int rr, ArrayOfCDI *pNewDirArray) {
  ...
  if (rr & STYLE_CHANGED) {
    OCPNMessageBox(
        NULL,
        _("Please restart AISNESIA to activate language or style changes."),
        _("AISNESIA Info"), wxOK | wxICON_INFORMATION);
  }
...
bool MyFrame::UpdateChartDatabaseInplace(ArrayOfCDI &DirArray, bool b_force,
  ...
  if (b_prog) {
    ...
    Updateprog->Create(_("AISNESIA Chart Update"), longmsg, 100, gFrame,
...
// initial frame and takes a while to initialize.  This gets aisnesia up and
// running much faster.
...
void ParseAllENC(wxWindow *parent) {
  ...
  if (1) {
    ...
    prog->Create(_("AISNESIA ENC Prepare"), "Longgggggggggggggggggggggggggggg",
