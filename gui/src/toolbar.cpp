 ...
 * \file
 *
 * Implement toolbar.h -- AISNESIA Toolbar
 */
    ...
    if (g_pi_manager) {
      ...
      for (unsigned int i = 0; i < tool_array.GetCount(); i++) {
        ...
        if (tool->GetId() == pttc->id) {
          aisnesia_plugin_113 *ppi =
              dynamic_cast<aisnesia_plugin_113 *>(pttc->m_pplugin);
          if (ppi) {
  ...
  if (m_parentContainer) {
    if (m_parentContainer->m_FloatingToolbarConfigMenu) {
      ToolbarChoicesDialog *dlg =
          new ToolbarChoicesDialog(NULL, m_parentContainer, -1, "AISNESIA",
...
ToolbarMOBDialog::ToolbarMOBDialog(wxWindow *parent)
    : wxDialog(parent, wxID_ANY, _("AISNESIA Alert"), wxDefaultPosition,
               wxSize(250, 230)) {
