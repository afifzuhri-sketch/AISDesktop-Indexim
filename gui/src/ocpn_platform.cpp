 ...
 * \file
 *
 * Implement ocpn_platform.h --  AISNESIA Platform specific support utilities
 */
...
bool OCPNPlatform::BuildGLCaps(void *pbuf) {
...
#ifndef __WXMSW__
  std::string gl_util_exe = "aisnesia-glutil";
#else
  std::string gl_util_exe = "aisnesia-glutil.exe";
#endif
  ...
  return true;
#else
  // The original codepath doing direct probing in the main AISNESIA process, now
...
// Add a new prefixes for search order.
  ...
  // "/usr". On the other hand, canonical installation prefix for AISNESIA is
  // "/usr/local".
  wxString locale_location;
  if (!wxGetEnv("AISNESIA_PREFIX", &locale_location)) {
  ...
  std::string macDir =
      ...
      "/Library/Application Support/AISNESIA/Contents/Resources";
...
//  and use this selection for aisnesia...
#if defined(__WXMSW__)
...
wxString OCPNPlatform::ChangeLocale(wxString &newLocaleID,
  ...
  if (isFlatpacked()) {
    ...
    path += "/.var/app/org.aisnesia.AISNESIA/data/locale";
  ...
  if (b_initok) {
    ...
    //  first, so that core aisnesia translations loaded later will become
    //  precedent.
    ...
    // Get core aisnesia catalog translation (.mo) file
    wxLogMessage("Loading catalog for aisnesia core.");
    locale->AddCatalog("aisnesia");
...
void OCPNPlatform::SetUpgradeOptions(wxString vNew, wxString vOld) {
  ...
  if (androidGetVersionCode() > g_AndroidVersionCode) {  // upgrade
    ...
    AndroidRemoveSystemFile(
        "/data/user/0/org.aisnesia.aisnesia/manPlug/libchartscale_pi.so");
  }
