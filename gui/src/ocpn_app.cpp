 ...
 * \file
 *
 * Implement ocpn_app.h -- AISNESIA main program
 */
...
const char *const kUsage =
    R"(Usage:
  aisnesia -h | --help
  aisnesia [-p] [-f] [-G] [-g] [-P] [-l <str>] [-u <num>] [-U] [-s] [GPX file ...]
  aisnesia --remote [-R] | -q] | -e] |-o <str>]

Options for starting aisnesia
  ...
  -P, --parse_all_enc          	Convert all S-57 charts to AISNESIA's internal format on start.
...
Options manipulating already started aisnesia
  -r, --remote                 	Execute commands on already running instance
  -R, --raise                  	Make running AISNESIA visible if hidden
  -q, --quit                   	Terminate already running aisnesia
  -e, --get_rest_endpoint      	Print rest server endpoint and exit.
  -o, --open=<GPX file>         Open file in running aisnesia
...
bool ShowNavWarning() {
  wxString msg(
      _("\n\
AISNESIA is distributed in the hope that it will be useful, \
but WITHOUT ANY WARRANTY; without even the implied \
warranty of MERCHANTABILITY or FITNESS FOR A \
PARTICULAR PURPOSE.\n\n\
See the GNU General Public License for more details.\n\n\
AISNESIA must only be used in conjunction with approved \
paper charts and traditional methods of navigation.\n\n\
DO NOT rely upon AISNESIA for safety of life or property.\n\n\
Please click \"Agree\" and proceed, or \"Cancel\" to quit.\n"));

  wxString vs = wxString::Format(" .. Version %s", VERSION_FULL);

#ifdef __ANDROID__
  androidShowDisclaimer(_("AISNESIA for Android") + vs, msg);
  return true;
#else
  msg.Replace("\n", "<br>");
  ...
  std::string title = _("Welcome to AISNESIA").ToStdString();
...
bool MyApp::OpenFile(const std::string &path) {
  ...
  if (!result) {
    ...
    s += std::string("\"") + path + "\"";
    wxMessageBox(s, "AISNESIA", wxICON_WARNING | wxOK);
    return false;
  }

  int wpt_dups;
  // Import with full vizibility of names and objects
  nav_objects.LoadAllGPXObjects(!nav_objects.IsAISNESIA(), wpt_dups, true);
...
void MyApp::OnInitCmdLine(wxCmdLineParser &parser) {
  // Add AISNESIA specific command line options. Help message
...
#else
static void ParseLoglevel(wxCmdLineParser &parser) {
  const char *strLevel = std::getenv("AISNESIA_LOGLEVEL");
...
bool MyApp::OnInit() {
  ...
  if (!g_bportable && wxDirExists(g_Platform->GetPrivateDataDir())) {
    ...
    if (m_checker.IsMainInstance()) {
      ...
      if (m_parsed_cmdline.action == CmdlineAction::Skip) {
        ...
        std::cerr << "No remote aisnesia found. Giving up.\n";
        m_exitcode = 1;
        return true;
      }
    } else {
      ...
      try {
        ...
        // Next start  will proceed normally. This may leave a zombie AISNESIA,
        // but at least O starts.
        m_checker.CleanUp();
        wxMessageBox(_("Sorry, an existing instance of AISNESIA may be too busy "
                       "to respond.\nPlease retry."),
                     "AISNESIA", wxICON_INFORMATION | wxOK);
        m_exitcode = 2;
  ...
  if (getenv("AISNESIA_FATAL_ERROR") != 0) {
    wxLogFatalError(getenv("AISNESIA_FATAL_ERROR"));
  }
  ...
  // the system menus.
  MyApp::SetAppDisplayName("AISNESIA");
  ...
  LOG_INFO("------- AISNESIA version %s restarted at %s -------\n", VERSION_FULL,
  ...
  //  Where are the aisnesia.mo files?
  ...
  //  This applies to either the system language, or to AISNESIA language
  //  selection
  ...
  // as allowing AISNESIA a percentage of available physical memory,
  // not to exceed 1 GB
...
#endif
  wxLogMessage(
      wxString::Format(_("AISNESIA Initialized in %ld ms."), init_sw.Time()));
  ...
  if (ipv4_addrs.size()) {
    ...
    StartMDNSService(g_hostname.ToStdString(), "aisnesia-object-control-service",
                     8000);
  }
...
void MyApp::BuildMainFrame() {
  ...
  wxString myframe_window_title = wxString("AISNESIA " + short_version_name);
  ...
  if (g_rebuild_gl_cache && g_bopengl && g_GLOptions.m_bTextureCompression &&
    ...
    // and is unusable until I restart aisnesia without generating the cache
    //        if( g_MainToolbar )
    //            g_MainToolbar->Hide();
  ...
  //    wxLogMessage( wxString::Format("AISNESIA Initialized in %ld ms.",
  //    init_sw.Time() ) );
}
...
void MyApp::LoadChartDatabase() {
  ...
  if (g_bFirstRun && (ChartDirArray.GetCount() == 0)) {
    ...
    wxRegKey RegKey(wxString("HKEY_LOCAL_MACHINE\\SOFTWARE\\AISNESIA"));
    if (RegKey.Exists()) {
      wxLogMessage(
...
int MyApp::OnExit() {
  wxLogMessage("aisnesia::MyApp starting exit.");
  m_checker.OnExit();
  m_usb_watcher.Stop();
  ...
  wxLogMessage("aisnesia::MyApp exiting cleanly...\n");
  wxLog::FlushActive();
