...
void ChartCanvas::SetGroupIndex(int index, bool autoSwitch) {
  ...
  if (!autoSwitch && bgroup_override) {
    ...
    OCPNMessageBox(this, msg, _("AISNESIA Group Notice"), wxICON_INFORMATION, 2);
  ...
  if (bgroup_override) {
    ...
    OCPNMessageBox(this, msg, _("AISNESIA Group Notice"), wxOK, 5);
  }
}
    ...
    for (auto pic : *PluginLoader::GetInstance()->GetPlugInArray()) {
      if (pic->m_enabled && pic->m_init_state) {
        switch (pic->m_api_version) {
          case 121: {
            auto *ppi = dynamic_cast<aisnesia_plugin_121 *>(pic->m_pplugin);
          ...
          if (pic->m_enabled && pic->m_init_state &&
            ...
            if (ptcmgr) {
              ...
              if (m_pIDXCandidate) {
                ...
                auto plugin =
                    dynamic_cast<aisnesia_plugin_121 *>(pic->m_pplugin);
      ...
      if (m_routeState)  // creating route?
      {
        ...
        if (pNearbyPoint && (pNearbyPoint != m_prev_pMousePoint) &&
          ...
          if (brp_viz) {
            ...
            m_FinishRouteOnKillFocus = false;
            int dlg_return =
                OCPNMessageBox(this, msg, _("AISNESIA Route Create"),
                ...
                if (m_routeState ==
                  ...
                  if (tail->GetIndexOf(pMousePoint) > 0) {  // Anything to do?
                    dlg_return = OCPNMessageBox(
                        this, dmsg, _("AISNESIA Route Create"),
                        (long)wxYES_NO | wxCANCEL | wxYES_DEFAULT);
                    m_FinishRouteOnKillFocus = true;
        ...
        if (m_pMouseRoute) {
          if (m_routeState == 1) {
            ...
            if (m_pMouseRoute->m_NextLegGreatCircle) {
              ...
              int answer = OCPNMessageBox(this, msg, _("AISNESIA Route Create"),
    ...
    if (g_btouch) {
      ...
      if (m_routeState)  // creating route?
      {
        ...
        if (pNearbyPoint && (pNearbyPoint != m_prev_pMousePoint) &&
          ...
          dlg_return = OCPNMessageBox(
              this, _("Use nearby waypoint?"), _("AISNESIA Route Create"),
              (long)wxYES_NO | wxCANCEL | wxYES_DEFAULT);
          m_FinishRouteOnKillFocus = true;
#else
          dlg_return = wxID_YES;
            ...
            if (procede) {
              ...
              if (m_routeState == 1) {  // first point in new route, preceeding
                ...
                if (tail->GetIndexOf(pMousePoint) != 1) {  // Anything to do?
                  dlg_return =
                      OCPNMessageBox(this, dmsg, _("AISNESIA Route Create"),
                                     (long)wxYES_NO | wxCANCEL | wxYES_DEFAULT);
                  m_FinishRouteOnKillFocus = true;
...
void ChartCanvas::UpdateCanvasS52PLIBConfig() {
  ...
  if (bSendPlibState) {
    ...
    //  S52PLIB state
    v["AISNESIA S52PLIB ShowText"] = GetShowENCText();
    v["AISNESIA S52PLIB ShowSoundings"] = GetShowENCDepth();
    v["AISNESIA S52PLIB ShowLights"] = GetShowENCLights();
    v["AISNESIA S52PLIB ShowAnchorConditions"] = m_encShowAnchor;
    v["AISNESIA S52PLIB ShowQualityOfData"] = GetShowENCDataQual();
    v["AISNESIA S52PLIB ShowATONLabel"] = GetShowENCBuoyLabels();
    v["AISNESIA S52PLIB ShowLightDescription"] = GetShowENCLightDesc();

    v["AISNESIA S52PLIB DisplayCategory"] = GetENCDisplayCategory();

    v["AISNESIA S52PLIB SoundingsFactor"] = g_ENCSoundingScaleFactor;
    v["AISNESIA S52PLIB TextFactor"] = g_ENCTextScaleFactor;
