...
private:
  void show_msg(wxString msg) {
    OCPNMessageBox(NULL, msg, wxString(_("AISNESIA Info")),
...
wxString message_by_status(PluginStatus stat) {
  switch (stat) {
    ...
    case PluginStatus::Managed:
      return _("Plugin is managed by AISNESIA");
    case PluginStatus::Unmanaged:
      return _("Plugin is not managed by AISNESIA");
    ...
    for (wxString str = manifest_file.GetFirstLine(); !manifest_file.Eof();
      ...
      if (str.Contains(pispec)) {
        ...
        if (!loader->CheckPluginCompatibility(str)) {
          wxString msg =
              _("The plugin is not compatible with this version of AISNESIA, "
...
void PlugInManager::ProcessLateInit(const PlugInContainer* pic) {
  if (pic->m_cap_flag & WANTS_LATE_INIT) {
    ...
    aisnesia_plugin_110* ppi = dynamic_cast<aisnesia_plugin_110*>(pic->m_pplugin);
    if (ppi) ppi->LateInit();
  }
}
...
bool PlugInManager::CheckBlacklistedPlugin(aisnesia_plugin* plugin) {
      ...
      if (pic->m_cap_flag & WANTS_OVERLAY_CALLBACK) {
        ...
        if (pdc)  // not in OpenGL mode
        {
          switch (pic->m_api_version) {
            case 106: {
              if (priority > 0) break;
              aisnesia_plugin_16* ppi =
                  dynamic_cast<aisnesia_plugin_16*>(pic->m_pplugin);
              if (ppi) ppi->RenderOverlay(*pdc, &pivp);
              break;
            }
            case 107: {
              if (priority > 0) break;
              aisnesia_plugin_17* ppi =
                  dynamic_cast<aisnesia_plugin_17*>(pic->m_pplugin);
              if (ppi) ppi->RenderOverlay(*pdc, &pivp);
              break;
            }
            case 108:
            ...
            case 115: {
              if (priority > 0) break;
              aisnesia_plugin_18* ppi =
                  dynamic_cast<aisnesia_plugin_18*>(pic->m_pplugin);
              if (ppi) ppi->RenderOverlay(*pdc, &pivp);
              break;
            }
            case 116:
            case 117: {
              ...
              aisnesia_plugin_116* ppi116 =
                  dynamic_cast<aisnesia_plugin_116*>(pic->m_pplugin);
              if (ppi116)
            ...
            case 121: {
              ...
              aisnesia_plugin_118* ppi118 =
                  dynamic_cast<aisnesia_plugin_118*>(pic->m_pplugin);
              if (ppi118)
...
void PlugInManager::SendBaseConfigToAllPlugIns() {
  ...
  wxJSONValue v;
  v["AISNESIA Version Major"] = VERSION_MAJOR;
  v["AISNESIA Version Minor"] = VERSION_MINOR;
  v["AISNESIA Version Patch"] = VERSION_PATCH;
  v["AISNESIA Version Date"] = VERSION_DATE;
  v["AISNESIA Version Full"] = VERSION_FULL;

  // Some useful display metrics
  if (g_MainToolbar) {
    v["AISNESIA Toolbar Width"] = g_MainToolbar->GetToolbarRect().width;
    v["AISNESIA Toolbar Height"] = g_MainToolbar->GetToolbarRect().height;
    v["AISNESIA Toolbar PosnX"] = g_MainToolbar->GetToolbarRect().x;
    v["AISNESIA Toolbar PosnY"] = g_MainToolbar->GetToolbarRect().y;
  }

  // Some rendering parameters
  v["AISNESIA Zoom Mod Vector"] = g_chart_zoom_modifier_vector;
  v["AISNESIA Zoom Mod Other"] = g_chart_zoom_modifier_raster;
  v["AISNESIA Scale Factor Exp"] =
      g_Platform->GetChartScaleFactorExp(g_ChartScaleFactor);
  v["AISNESIA Display Width"] = (int)g_display_size_mm;
  v["AISNESIA Content Scale Factor"] = OCPN_GetDisplayContentScaleFactor();
  v["AISNESIA Display DIP Scale Factor"] = OCPN_GetWinDIPScaleFactor();

  wxJSONWriter w;
  wxString out;
  w.Write(v, out);
  SendMessageToAllPlugins(wxString("AISNESIA Config"), out);
}
...
void PlugInManager::SendS52ConfigToAllPlugIns(bool bReconfig) {
  ...
  //  S52PLIB state
  if (ps52plib) {
    //         v["AISNESIA S52PLIB ShowText"] = ps52plib->GetShowS57Text();
    //         v["AISNESIA S52PLIB ShowSoundings"] =
    //         ps52plib->GetShowSoundings(); v["AISNESIA S52PLIB ShowLights"]
    //         = !ps52plib->GetLightsOff();
    v["AISNESIA S52PLIB ShowAnchorConditions"] = ps52plib->GetAnchorOn();
    v["AISNESIA S52PLIB ShowQualityOfData"] = ps52plib->GetQualityOfData();
    //         v["AISNESIA S52PLIB DisplayCategory"] =
    //         ps52plib->GetDisplayCategory();

    // Global parameters
    v["AISNESIA S52PLIB MetaDisplay"] = ps52plib->m_bShowMeta;
    v["AISNESIA S52PLIB DeclutterText"] = ps52plib->m_bDeClutterText;
    v["AISNESIA S52PLIB ShowNationalText"] = ps52plib->m_bShowNationalTexts;
    v["AISNESIA S52PLIB ShowImportantTextOnly"] =
        ps52plib->m_bShowS57ImportantTextOnly;
    v["AISNESIA S52PLIB UseSCAMIN"] = ps52plib->m_bUseSCAMIN;
    v["AISNESIA S52PLIB UseSUPER_SCAMIN"] = ps52plib->m_bUseSUPER_SCAMIN;
    v["AISNESIA S52PLIB SymbolStyle"] = ps52plib->m_nSymbolStyle;
    v["AISNESIA S52PLIB BoundaryStyle"] = ps52plib->m_nBoundaryStyle;
    v["AISNESIA S52PLIB ColorShades"] = S52_getMarinerParam(S52_MAR_TWO_SHADES);
    v["AISNESIA S52PLIB Safety Depth"] =
        (double)S52_getMarinerParam(S52_MAR_SAFETY_DEPTH);
    v["AISNESIA S52PLIB Shallow Contour"] =
        (double)S52_getMarinerParam(S52_MAR_SHALLOW_CONTOUR);
    v["AISNESIA S52PLIB Deep Contour"] =
        (double)S52_getMarinerParam(S52_MAR_DEEP_CONTOUR);
  }

  // Notify plugins that S52PLIB may have reconfigured global options
  v["AISNESIA S52PLIB GlobalReconfig"] = bReconfig;
...
aisnesia_plugin* PlugInManager::FindToolOwner(const int id) {
  for (unsigned int i = 0; i < m_PlugInToolbarTools.GetCount(); i++) {
...
wxString PlugInManager::GetToolOwnerCommonName(const int id) {
  aisnesia_plugin* ppi = FindToolOwner(id);
  if (ppi) {
    auto plugin_array = PluginLoader::GetInstance()->GetPlugInArray();
    for (unsigned int i = 0; i < plugin_array->GetCount(); i++) {
      PlugInContainer* pic = plugin_array->Item(i);
...
aisnesia_plugin* PlugInManager::GetProvidingPlugin(
    const wxString& ChartClassName) {
  aisnesia_plugin* plugin = nullptr;
  auto plugin_array = PluginLoader::GetInstance()->GetPlugInArray();
  for (unsigned int i = 0; i < plugin_array->GetCount(); i++) {
    PlugInContainer* pic = plugin_array->Item(i);
    if (pic && pic->m_enabled && pic->m_init_state &&
...
#define DISABLED_SETTINGS_MSG                                               \
  _("These settings might destabilize AISNESIA and are by default disabled." \
...
static const char* const DOWNLOAD_REPO_PROTO =
    "https://raw.githubusercontent.com/AISNESIA/plugins/@branch@/"
    "ocpn-plugins.xml";
...
void CatalogMgrPanel::OnUpdateButton(wxCommandEvent& event) {
  ...
  if (status != CatalogHandler::ServerStatus::OK) {
    ...
    OCPNMessageBox(this, message, _("AISNESIA Catalog update"),
                   wxICON_ERROR | wxOK);
    return;
  }
  ...
  if (!AndroidSecureCopyFile(wxString(filePath.c_str()),
    ...
    OCPNMessageBox(this, _("Unable to copy catalog file"),
                   _("AISNESIA Catalog update"), wxICON_ERROR | wxOK);
    return;
  }
#else
  ...
  OCPNMessageBox(this, _("Catalog update successful"),
                 _("AISNESIA Catalog update"), wxICON_INFORMATION | wxOK);
}
...
void CatalogMgrPanel::OnTarballButton(wxCommandEvent& event) {
  ...
  if (!ok) {
    OCPNMessageBox(
        ...
        _("AISNESIA Plugin Import Error"));
    return;
  }
...
PluginPanel::PluginPanel(wxPanel* parent, const std::string& name)
  ...
  m_info_btn = new WebsiteButton(this, "https:\\aisnesia.org");
...
wxString GetPlugInPath(aisnesia_plugin* pplugin) {
  wxString ret_val;
  auto loader = PluginLoader::GetInstance();
