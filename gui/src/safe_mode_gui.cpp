...
static const char* LAST_RUN_ERROR_MSG =
    _("<p>The last aisnesia run seems to have failed. Do you want to run\n"
      "in safe mode without plugins and other possibly problematic\n"
      "features?\n</p><br/></br><p>You may consider visiting the <a "
      "href=\"https://github.com/AISNESIA/AISNESIA/wiki/"
      "AISNESIA-5.10-known-issues\">list of known issues</a>.</p>");
...
void check_last_start() {
  ...
  if (!ocpn::exists(path)) {
    ...
    dest << "Internal aisnesia use" << std::endl;
    dest.close();
    return;
  }
