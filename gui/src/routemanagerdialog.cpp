...
void RouteManagerDialog::OnRteDeleteAllClick(wxCommandEvent &event) {
  int dialog_ret =
      OCPNMessageBox(this, _("Are you sure you want to delete <ALL> routes?"),
                     wxString(_("AISNESIA Alert")), wxYES_NO);
...
void RouteManagerDialog::OnRteSendToPeerClick(wxCommandEvent &event) {
  ...
  if (!list.empty()) {
    ...
    dlg.Create(NULL, -1, _("Send Route(s) to AISNESIA Peer") + "...", "");
    dlg.ShowModal();
  }
}
...
void RouteManagerDialog::OnWptSendToPeerClick(wxCommandEvent &event) {
  ...
  if (!list.empty()) {
    ...
    dlg.SetScanTime(5);  // seconds
    dlg.Create(NULL, -1, _("Send Waypoint(s) to AISNESIA Peer") + "...", "");
    dlg.ShowModal();
  }
}
...
void RouteManagerDialog::OnTrkSendToPeerClick(wxCommandEvent &event) {
  ...
  if (!list.empty()) {
    ...
    dlg.SetScanTime(5);  // seconds
    dlg.Create(NULL, -1, _("Send Track(s) to AISNESIA Peer") + "...", "");
    dlg.ShowModal();
  }
}
  ...
  switch (event.GetId()) {
    case TRACK_CLEAN: {
      ...
      wxString msg = wxString::Format(
          ...
          reduction);
      OCPNMessageBox(this, msg, _("AISNESIA info"), wxICON_INFORMATION | wxOK);
...
void RouteManagerDialog::TrackToRoute(Track *track) {
  ...
  wxGenericProgressDialog pprog(_("AISNESIA Converting Track to Route...."),
...
void RouteManagerDialog::OnWptDeleteAllClick(wxCommandEvent &event) {
  ...
  int answer =
      OCPNMessageBox(this, prompt, wxString(_("AISNESIA Alert")), buttons);
...
void RouteManagerDialog::OnLayDeleteClick(wxCommandEvent &event) {
  ...
  if (wxFileExists(destf)) {
    prompt.Append("\n");
    prompt.Append(
        _("The file will also be deleted from AISNESIA's layer directory."));
    prompt.Append("\n (" + destf + ")");
    ispers = true;
  }
  int answer =
      OCPNMessageBox(this, prompt, wxString(_("AISNESIA Alert")), wxYES_NO);
