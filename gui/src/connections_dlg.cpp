...
static const char* kInfoHeader = _("AISNESIA help").c_str();
static const char* kInfo = _(R"---(
Normally AISNESIA sends the RMB (distance and heading to waypoint
    ...
    explicit TalkerIdRow(wxWindow* parent) : wxPanel(parent) {
      ...
      text_ctrl->SetToolTip(
          _("Enter a two-letter talker ID to override the default ID in NMEA "
            "sentences generated by AISNESIA (e.g., GP, HC, EC). This affects "
            "only sentences created by AISNESIA, not those forwarded from other "
            "devices."));
