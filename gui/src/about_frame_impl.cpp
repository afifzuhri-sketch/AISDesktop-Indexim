...
AboutFrameImpl::AboutFrameImpl(wxWindow* parent, wxWindowID id,
  ...
  wxBitmap logo(wxString::Format("%s/aisnesia.png",
...
void AboutFrameImpl::OnLinkHelp(wxHyperlinkEvent& event) {
  ...
  if (!::wxFileExists(testFile)) {
    wxString msg = _("AISNESIA Help documentation is not available locally.");
    msg += "\n";
    msg +=
        _("Would you like to visit the aisnesia.org website for more "
          "information?");

    if (wxID_YES ==
        OCPNMessageBox(NULL, msg, _("AISNESIA Info"), wxYES_NO | wxCENTER, 60)) {
      wxLaunchDefaultBrowser("https://aisnesia.org");
    }
  } else
#endif
  {
