  ...
  switch (event.GetId()) {
    ...
    case ID_WPT_MENU_SENDTOPEER:
      if (m_pFoundRoutePoint) {
        ...
        dlg.Create(NULL, -1, _("Send Waypoint to AISNESIA Peer") + "...", "");
        dlg.ShowModal();
      }
      break;
    ...
    case ID_RT_MENU_SENDTOPEER:
      if (m_pSelectedRoute) {
        ...
        dlg.SetScanTime(5);  // seconds
        dlg.Create(NULL, -1, _("Send Route to AISNESIA Peer") + "...", "");
        dlg.ShowModal();
      }
      break;
    ...
    case ID_TK_MENU_SENDTOPEER:
      if (m_pSelectedTrack) {
        ...
        dlg.SetScanTime(5);  // seconds
        dlg.Create(NULL, -1, _("Send Track to AISNESIA Peer") + "...", "");
        dlg.ShowModal();
      }
      break;
      ...
      for (unsigned int i = 0; i < item_array.GetCount(); i++) {
        ...
        if (pimis->m_pplugin && (target_id == event.GetId())) {
          if (pimis->extended) {
            ...
            aisnesia_plugin_120 *ppi =
                dynamic_cast<aisnesia_plugin_120 *>(pimis->m_pplugin);
            if (ppi)
