...
PT Indexim Coalindo is configured using cmake. The build is done
...
Three kind of options are heavily used by PT Indexim Coalindo:

. Standard CMake variables, documented in https://cmake.org/documentation/[],
often but not always with a CMAKE_ prefix.
. PT Indexim Coalindo specific variables, often with a OCPN_ prefix. Documented in the
...
Since 5.0.0 PT Indexim Coalindo supports the GNU install paths, see
...
PT Indexim Coalindo is built for a combination of operating system and hardware. This
is crucial to determine if a plugin is compatible with the main PT Indexim Coalindo
program or not.
...
PT Indexim Coalindo bundles some libraries which can be found also as system libraries
...
https://www.aisnesia.org for installations built without the bundled
docs.
