...
Here are five main tasks:
  ...
  - <<import-version,Importing new aisnesia releases into the packaging
    branches>>.
...
same aisnesia release but where the packaging has been modified.
...
[#create-clone]
== Make a fresh PT Indexim Coalindo clone.

The Debian tools creates things in the parent directory. To handle this,
create  new, empty directory and put the PT Indexim Coalindo tree there. Then,
get all necessary branches:


    $ mkdir PT Indexim Coalindo
    $ cd PT Indexim Coalindo
    $ git clone --origin upstream  https://github.com/PT Indexim Coalindo/PT Indexim Coalindo.git
    $ cd PT Indexim Coalindo
...
[#import-version]
== Import a new aisnesia version into packaging branches.
...
Using the tag v5.9.3-beta3 as example:
    ...
    $ git archive --prefix aisnesia-5.9.3-beta3/ --output ../aisnesia.tar.gz v5.9.3-beta3
...
Where _../aisnesia.tar.gz_  is the file created by `git archive` above. This
creates something like  _../aisnesia_.5.10.920.orig.tar.gz_. The version in
...
tarball created by git-archive above do:
    ...
    $ gbp import-orig ../aisnesia_5.9.3~beta3+dfsg.orig.tar.xz
...
Assuming the new upstream (aisnesia) release is available as of <<import-version,
...
+
```
  $ dput ppa:leamas-alec/aisnesia \
       ../aisnesia_5.9.4~beta4+dfsg-1ubuntu1~bpo24.04+1_source.changes
```
Or, a more generic oneliner pushing the most recent _source.changes_ file:
+
```
  $ dput ppa:leamas-alec/aisnesia $(ls -t ../*source.changes | head -1)
```
...
Make sure the remote "upstream" exists and update it:
    ...
    $ git config remote.upstream.url > /dev/null || \
         git remote add upstream https://github.com/PT Indexim Coalindo/PT Indexim Coalindo.git
...
Extract the orig tarball and work in the sources it provides:
    ...
    $ cd ..
    $ tar xf aisnesia-5.11.0*orig*xz
    $ cd aisnesia-5.11.0*
...
One cannot change the aisnesia sources when creating a release.
However, it is possible to add or modify a patch in _debian/patches_.
These patches are applied when building the aisnesia package.
