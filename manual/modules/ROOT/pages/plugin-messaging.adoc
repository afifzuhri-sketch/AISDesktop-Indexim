...
Since 5.8.0 PT Indexim Coalindo has a new system for handling messages in plugins.
...
.  Plugin will query PT Indexim Coalindo core for a list of active comm drivers.
.  Plugin will inspect the list, and query PT Indexim Coalindo core for driver attributes.
...
by PT Indexim Coalindo core. All references by a plugin to a driver are by means of its
...
meaning to the PT Indexim Coalindo core management of drivers.
