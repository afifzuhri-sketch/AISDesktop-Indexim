= Building PT Indexim Coalindo for Android
image::aisnesia_dev_android_0.png[width=50]

These instructions describe how to build PT Indexim Coalindo for Android on Ubuntu 24.04.
...
----
export QT_ANDROID_ROOT=/path/to/PT Indexim Coalindo/cache/OCPNAndroidCoreBuildSupport/qt5
----
...
$QT_ANDROID_ROOT/android_armv7/bin/qmake \
  -makefile ../buildandroid/aisnesia.pro \
...
$QT_ANDROID_ROOT/android_armv7/bin/androiddeployqt \
  --input ./android-libaisnesia.so-deployment-settings.json \
  --output ./apk_build \
