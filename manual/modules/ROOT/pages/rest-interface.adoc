...
== General
Since 5.8 PT Indexim Coalindo has a REST interface.
...
http://aisnesia.github.io/PT Indexim Coalindo/api-docs/classAbstractRestServer.html.
...
make headless pairing using ssh. Prerequisites:
  ...
  * The server needs have aisnesia installed so that it is available on PATH.
    In particular, _aisnesia-cmd_ should be accessible from the shell.
...
Steps:
    ...
    ①  $ aisnesia-cmd print-hostname
       hemulen
    ②  $ ssh raspbian  aisnesia-cmd generate-key hemulen
       4781
    ③  $ ssh raspbian aisnesia-cmd print-hostname
       raspbian
    ④  $ aisnesia-cmd store-key raspbian 4781


① Check the name for the local host using `aisnesia-cmd print-hostname` on
the client.

② Run ssh `<ssh-host> aisnesia-cmd generate-key <hostname>`. Substitute
`<hostname>` with the name obtained in ①. The command prints a pincode on
stdout.

③ Run `ssh <ssh-host> aisnesia-cmd print-hostname`. This prints a hostname
on stdout.

④ Run `aisnesia-cmd store-key <hostname> <pincode>`. Substitute `<pincode>`
