...
This section provides guidance for PT Indexim Coalindo core developers who are creating or extending the plugin API. Unlike other parts of the developer manual that focus on plugin developers using the API, this section covers best practices, design principles, and implementation considerations for those maintaining and evolving the API itself.

The plugin API serves as the contract between PT Indexim Coalindo core and its plugins. Any changes to this API must be carefully considered to maintain backward compatibility while enabling new features. As a core developer, your decisions about the plugin API will affect both the stability of existing plugins and the capabilities available to future plugin authors.
...
* Plugins compiled against an older version of the API should continue to work with newer versions of PT Indexim Coalindo.
...
* Once a new PT Indexim Coalindo version has been published along with a new version of the plugin API, do not:
...
----
// Good
bool UpdateSingleWaypoint(const PlugIn_Waypoint& waypoint);

// Bad
bool UpdateSingleWaypoint(PlugIn_Waypoint waypoint);
----
...
Up to API 1.20 (PT Indexim Coalindo 5.12)  all API methods and types was in in global namespace. 
